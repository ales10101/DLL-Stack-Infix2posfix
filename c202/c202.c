...
void Stack_Init( Stack *stack ) {
	if(!stack){ //checks for validity of pointer
...
bool Stack_IsFull( const Stack *stack ) {
	return (stack->topIndex == (STACK_SIZE - 1)) ? 1 : 0; //one if stack is empty, zero else
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	if(stack && dataPtr){ 
		if(Stack_IsEmpty(stack)){ //it is not forbidden to use already implemented functions in this assignment
